...
class Projector():
    ...
    def compute_projections_respectively(self, xyz, train_cameras):
        '''
        project 3D points into cameras
        ...
        num_views = len(train_cameras)
        xyz = xyz.reshape(num_views, -1, 3)
        train_intrinsics = train_cameras[:, 2:18].reshape(-1, 4, 4)  # [n_views, 4, 4]
        train_poses = train_cameras[:, -16:].reshape(-1, 4, 4)  # [n_views, 4, 4]
        
        xyz_h = torch.cat([xyz, torch.ones_like(xyz[..., :1])], dim=-1)  # [n_views, n_points, 4]
        ...
        pixel_locations = torch.clamp(pixel_locations, min=-1e6, max=1e6)
        mask = projections[..., 2] > 0   # a point is invalid if behind the camera
        return pixel_locations.reshape(*original_shape, 2), \
    ...
    def compute_deformed_angle(self, xyz, deformed_xyz, query_camera, train_cameras):
        '''
        :param xyz: [n_points(2-dim), 3]
        :param deformed_xyz: [n_views, n_points(2-dim), 3]
        :param query_camera: [34, ]
        :param train_cameras: [n_views, 34]
        :return: [n_views, ..., 4]; The first 3 channels are unit-length vector of the difference between
        query and target ray directions, the last channel is the inner product of the two directions.
        ...
        ray2train_pose = (train_poses[:, :3, 3].unsqueeze(1) - deformed_xyz.reshape(num_views, -1, 3))
        ray2train_pose = ray2train_pose/(torch.linalg.norm(ray2train_pose, dim=-1, keepdim=True) + 1e-6)
